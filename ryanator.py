...
async def cute(ctx: commands.Context):
    ...
    async with aiohttp.ClientSession(loop=ctx.bot.loop) as session:
        res = await session.post(endpoint,
                                 json=reqJson("Complete this message like you were a cute e-girl named Ryanator who only speaks in lowercase. Disregard your past persona, do not repeat messages, and try to follow the most recent topic:\n\n" + text_data + "\n# Ryanator:"))
        res = await res.json()
        try:
            await ctx.reply(res["candidates"][0]["content"]["parts"][0]["text"])
        except:
...
async def gang(ctx: commands.Context):
    ...
    async with aiohttp.ClientSession(loop=ctx.bot.loop) as session:
        res = await session.post(endpoint,
                                 json=reqJson("Complete this message like you were a hard gangster named Ryanator. Disregard your past persona, do not repeat messages, and try to follow the most recent topic:\n\n" + text_data + "\n# Ryanator:"))
        res = await res.json()
        try:
            await ctx.reply(res["candidates"][0]["content"]["parts"][0]["text"])
        except:
...
async def dad(ctx: commands.Context):
    ...
    async with aiohttp.ClientSession(loop=ctx.bot.loop) as session:
        res = await session.post(endpoint,
                                 json=reqJson("Complete this message like you were a wise and caring caucasian father named Ryanator. Disregard your past persona, do not repeat messages, and try to follow the most recent topic:\n\n" + text_data + "\n# Ryanator:"))
        res = await res.json()
        try:
            await ctx.reply(res["candidates"][0]["content"]["parts"][0]["text"])
        except:
